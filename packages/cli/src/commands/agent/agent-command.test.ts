 ...
 * - §4.5 Agent New — Package-Driven (EARS-E1 to E7, G20)
 */
...
describe('AgentCommand', () => {
  ...
  describe('4.1. Agent Run (EARS-A1 to A6)', () => {
    ...
    it('[EARS-A5] should read input from --input-file', async () => {
      // Write a real temp file to test file reading
      ...
      const tmpFile = nodePath.join(nodeOs.tmpdir(), `agent-test-input-${Date.now()}.json`);
      nodeFs.writeFileSync(tmpFile, '{"fromFile": true, "count": 42}');

      try {
        await agentCommand.executeRun('test-echo', createRunOptions({ inputFile: tmpFile }));

        expect(mockAgentRunnerModule.runOnce).toHaveBeenCalledWith(
          expect.objectContaining({
            input: expect.objectContaining({ fromFile: true, count: 42 }),
          }),
        );
      } finally {
  ...
  describe('4.2. Agent List (EARS-B1 to B3)', () => {
    ...
    it('[EARS-B2] should filter agents by --engine type', async () => {
      ...
      expect(allCalls).not.toContain('jira-manager');
    });
  ...
  describe('4.5. Agent New — Package-Driven Registration (EARS-E1 to E7)', () => {
    ...
    afterEach(() => {
      ...
      mockConsoleErrorLocal.mockRestore();
    ...
    it('[EARS-E5] should auto-create ActorRecord with Ed25519 when not exists', async () => {
      tmpAgentDir = createFakeAgent({
        name: '@gitgov/agent-security-audit',
        main: 'dist/index.mjs',
      ...
      expect(mockIdentityAdapter.createActor).toHaveBeenCalledWith(
        expect.objectContaining({ type: 'agent', displayName: 'security-audit' }),
        'self',
      );
      expect(mockAgentAdapter.createAgentRecord).toHaveBeenCalledTimes(2);
    });

    it('[EARS-E3] should fail with descriptive error when gitgov.agent field is missing', async () => {
    ...
    it('[EARS-E4] should warn when required env vars from gitgov.agent.env are missing', async () => {
      tmpAgentDir = createFakeAgent({
        ...
        main: 'dist/index.mjs',
        gitgov: { agent: { purpose: 'review', function: 'runReviewAdvisor', env: ['LLM_API_KEY'] } },
      });
      ...
      expect(mockConsoleErrorLocal).toHaveBeenCalledWith(
        expect.stringContaining('LLM_API_KEY is required'),
      );
    });

    it('[EARS-E7] should show success message with derived actorId from package name', async () => {
      tmpAgentDir = createFakeAgent({
        name: '@gitgov/agent-security-audit',
        main: 'dist/index.mjs',
        gitgov: { agent: { purpose: 'audit', function: 'runAgent' } },
    ...
    it('[EARS-E6] should update existing agent without creating duplicates', async () => {
      tmpAgentDir = createFakeAgent({
        name: '@gitgov/agent-security-audit',
        main: 'dist/index.mjs',
        gitgov: { agent: { purpose: 'audit', function: 'runAgent' } },
