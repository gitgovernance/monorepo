import { MemoryRecordStore } from './memory_record_store';
...
describe('MemoryRecordStore', () => {
  let store: MemoryRecordStore<TestRecord>;

  beforeEach(() => {
    store = new MemoryRecordStore<TestRecord>();
  });
  ...
  // ─────────────────────────────────────────────────────────
  // §4.2 MemoryRecordStore-Specific Behavior (EARS-B1 a B4)
  // ─────────────────────────────────────────────────────────

  describe('MemoryRecordStore-Specific Behavior (EARS-B1 to B4)', () => {
    it('[EARS-B1] should initialize with empty Map', () => {
      const freshStore = new MemoryRecordStore<TestRecord>();
    ...
    it('[EARS-B4] should use direct reference when deepClone is false', async () => {
      const noCloneStore = new MemoryRecordStore<TestRecord>({ deepClone: false });
      const record: TestRecord = { id: 'test-1', name: 'Test', data: { nested: 'value' } };
  ...
  describe('Test Helpers', () => {
    ...
    it('should accept initial data', () => {
      ...
      const preloadedStore = new MemoryRecordStore<TestRecord>({ initial });
